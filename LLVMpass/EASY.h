...
namespace EASY {
    ...
    private:
        ...
        std::fstream bpl;
        //Function *threadFunc;
        std::vector<Function *> threadFunc;
        std::vector<int> threadNum;  // save threadnum of each threadfunc
        int partitionNum = 0;
        bool partition_flag;
        ...
        std::vector<std::vector<Instruction *>  > bankAddressInstrList2d;  // [no. threadfunc][bankAddressInstrList]
       ...
       //                         int &numElem, Type **elemTy);
        ...
        void printFunctionPrototype(Function *F, std::fstream *fout, int &count);
        void anlyzePhiInstr(Module &M);
        void printVarDeclarations(Function *F, std::fstream *fout);
        void printVarDeclarationsInMain(Function *F, std::fstream *fout, std::vector<int>& threadNum);
        void instrDecoding(Function *F, std::fstream *fout, int &count);
        void instrDecodingInMain(Function *F, std::fstream *fout, std::vector<int>& threadNum);
