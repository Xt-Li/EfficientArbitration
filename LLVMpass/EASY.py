...
# find thread function, thread number, MUX number and partition number
inLLVM.seek(0)
...
for line in inLLVM:
    ...
    if addrPFuncHeader != -1:
        ...
        funcTemp = line[addrPFuncHeader+16 : addrPFuncName]  # get funcName
        if funcName.count(funcTemp) == 0:   # new function name
            funcNum += 1
            threadNum.append(1)
            funcName.append(funcTemp)
        else:                             
            threadNum[funcNum] += 1
print("\nFound the number of thread functions "+str(funcNum+1))
i = 0
while i < len(funcName):
    print("\nFound thread function name \"" + funcName[i] +"\" with " + str(threadNum[0]) + " threads.")
    i += 1
...
for line in inLLVM:
    ...
    if line.startswith("  %GEP_sub_") and funcAddr == 0:
        ...
        if gFlag == 0:
            indexNum += 1
            print("\nindex number in LLVM: "+str(indexNum)+"\n")
        gFlag = 1
    else:
        gFlag = 0
...
if sum(threadNum) != max(thd)+1:
    err += 1
    print("Error: Thread number does not match: has " + str(max(thd)+1) + " in Boogie and " + str(sum(threadNum)) + " in LLVM. \n")
...
valid = [0] * len(thd)   # threads to remove
...
for line in inLLVM:
    ...
    if line.startswith("  %GEP_sub_"):
        ...
        while line[startAddr] != "_":
            startAddr -= 1
        startAddr += 1
        parIndex = int(line[startAddr:endAddr])   #find gep_sub_0/1/2/3
        # print(str(parIndex))
        # print ("\nfuncAddr " + str(funcAddr))
        #print ("\nindexAddr " + str(indexAddr))
        #print ("\nparIndex " + str(parIndex))
